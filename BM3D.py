...
if __name__ == '__main__':
    ...
    path = './data_add_noise15/'
    path_result  = './data_result_15/'

    #================================== Parameters initialization ==================================
    
    sigma = 15 # variance of the noise
    ...
    for i in range(160):
        image_name = path + os.sep + str(i+1) + '.png'
        noisy_img = cv2.imread(image_name)
        noisy_img = cv2.cvtColor(noisy_img, cv2.COLOR_BGR2GRAY)
        basic_img = BM3D_Step1(noisy_img)
        
        basic_img_uint = np.zeros(noisy_img.shape)
        ...
        image_reuslt_name = path_result + os.sep + str(i+1) + '.png'
        cv2.imwrite(image_reuslt_name, final_img)
        print('%d th process' % (i+1))
